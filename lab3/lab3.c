...
//  
//
//  Created by Albert Wu on 2/7/19.
//
...
int executeCommand(char *onecmd, int readfd, int writefd, char *env[]) {
  ...
  if (i > 3 && (strcmp(myargvs[i - 2], ">") == 0 || strcmp(myargvs[i - 2], ">>"))) {
  ...
  while(1){
    ...
    if(strcmp(myargvs[0], "cd") == 0) {
      ...
      if(pid) {
        ...
        // iteratively spawn child for each command, linked by pipe
        int fd[2];
        int readfd = -1;
        int nextreadfd = -1;
        int writefd = -1;
